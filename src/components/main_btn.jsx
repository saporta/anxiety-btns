...
import ambientSound from "../assets/crowd.mp3";
...
export default function MainBtn({ onClicked }) {
    ...
    useEffect(() => {
        ...
        // Add a one-time global click listener to start audio if blocked
        window.addEventListener('click', playAudio, { once: true });

        // Listen for the stop event from SecBtn
        const stopAudio = () => audio.pause();
        window.addEventListener("stop-ambient-sound", stopAudio);

        return () => {
            audio.pause();
            audio.src = "";
            window.removeEventListener('click', playAudio);
            window.removeEventListener("stop-ambient-sound", stopAudio);
        };
    }, []);
